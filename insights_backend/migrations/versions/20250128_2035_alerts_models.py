...
Revision ID: f764c6ac1ae0
Revises: c830f7ff651e
Create Date: 2025-01-28 20:35:51.629704
...
def upgrade() -> None:
    ...
    op.create_index(op.f("ix_insights_store_alert_tags"), "alert", ["tags"], unique=False, schema="insights_store")
    op.create_index(
    ...
    op.execute(
        "CREATE POLICY tenant_isolation_insights_store_notificationexecution ON insights_store.notificationexecution USING (tenant_id = current_setting('app.current_tenant')::int);"
    )
    op.execute("ALTER TABLE insights_store.alert ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "CREATE POLICY tenant_isolation_insights_store_alert ON insights_store.alert USING (tenant_id = current_setting('app.current_tenant')::int);"
    )
    op.execute("ALTER TABLE insights_store.notificationchannelconfig ENABLE ROW LEVEL SECURITY;")
    op.execute(
        "CREATE POLICY tenant_isolation_insights_store_notificationchannelconfig ON insights_store.notificationchannelconfig USING (tenant_id = current_setting('app.current_tenant')::int);"
    )
...
def downgrade() -> None:
    ...
    op.drop_index(op.f("ix_insights_store_alert_tags"), table_name="alert", schema="insights_store")
