...
async def test_list_combined_channels_and_users(async_client: AsyncClient, mocker, insert_tenant):
    # Mock the combined listing with both channels and users
    mock_combined = {
        "results": [
            {"name": "general", "id": "C9787896", "is_channel": True},
            {"name": "random", "id": "C5443424", "is_channel": True},
            {"name": "user1", "id": "U12345", "is_channel": False, "is_dm": True},
            {"name": "user2", "id": "U67890", "is_channel": False, "is_dm": True},
        ],
        "next_cursor": None,
    }
    mocker.patch("commons.clients.slack.SlackClient.list_channels", return_value=mock_combined)

    # Act
    response = await async_client.get("/v1/slack/channels", params={"include_users": True})
    ...
    # Check that we have both channels and users
    channels = [item for item in data["results"] if item.get("is_channel", True)]
    users = [item for item in data["results"] if item.get("is_channel", False)]

    assert len(channels) == 2
    assert len(users) == 2


async def test_filter_by_name_combined(async_client: AsyncClient, mocker, insert_tenant):
    # Mock the response when filtering by name in combined results
    mock_filtered = {
        "results": [
            {"name": "test-channel", "id": "C0987876", "is_channel": True, "is_dm": False},
            {"name": "test-user", "id": "U12345", "is_channel": False, "is_dm": True},
        ],
        "next_cursor": None,
    }
    mocker.patch("commons.clients.slack.SlackClient.list_channels", return_value=mock_filtered)

    # Act
    response = await async_client.get("/v1/slack/channels", params={"include_users": True, "name": "test"})

    # Assert
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert len(data["results"]) == 2
    assert all("test" in item["name"] for item in data["results"])

    # Verify we have both a channel and a user in results
    assert any(item.get("is_channel", True) for item in data["results"])
    assert any(item.get("is_dm", True) for item in data["results"])
