...
class TestForecastingEvaluator:
    ...
    async def test_evaluate_all_story_types(self, evaluator_with_series, mock_forecasting_pattern, mock_metric):
        ...
        for story in stories:
            ...
            assert story["story_group"] == StoryGroup.REQUIRED_PERFORMANCE if story["story_type"] == StoryType.REQUIRED_PERFORMANCE else StoryGroup.LIKELY_STATUS
    ...
    def test_create_forecasted_on_track_story(
        ...
        assert story["story_group"] == StoryGroup.LIKELY_STATUS if story["story_type"] != StoryType.REQUIRED_PERFORMANCE else StoryGroup.REQUIRED_PERFORMANCE
        assert story["genre"] == StoryGenre.PERFORMANCE
        assert story["metric_id"] == "test_metric"
        assert "title" in story
        assert "detail" in story
