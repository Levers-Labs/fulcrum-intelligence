...
def test_story_factory_create_story_builder_growth(mock_query_service, mock_analysis_service, mock_db_session):
    story_builder = StoryFactory.create_story_builder(
        StoryGroup.GROWTH_RATES, mock_query_service, mock_analysis_service, mock_db_session
    )
...
def test_story_factory_create_story_builder_unsupported_group(
    ...
    with pytest.raises(ValueError) as excinfo:
        StoryFactory.create_story_builder(
            "UNSUPPORTED_GROUP", mock_query_service, mock_analysis_service, mock_db_session
        )
