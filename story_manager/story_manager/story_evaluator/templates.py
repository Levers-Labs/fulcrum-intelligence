...
STORY_TEMPLATES = {
    StoryType.ON_TRACK: {
        ...
        "detail": "{{ metric.label }} is at {{ current_value|format_with_unit(metric.unit) }}, "
        "{{ trend_direction }} {{ change_percent|format_percent }}% {{ pop }} "
        "and beating its target of {{ target_value|format_with_unit(metric.unit) }} "
    ...
    StoryType.OFF_TRACK: {
        ...
        "detail": "{{ metric.label }} is at {{ current_value|format_with_unit(metric.unit) }}, "
        "{{ trend_direction }} {{ change_percent|format_percent }}% {{ pop }} "
        "and missing its target of {{ target_value|format_with_unit(metric.unit) }} "
    ...
    StoryType.RECORD_HIGH: {
        ...
        "detail": "The {{ grain_label }} value for {{ metric.label }} of {{ high_value|format_with_unit(metric.unit) "
        "}} is now the {%"
    ...
    StoryType.RECORD_LOW: {
        ...
        "detail": "The {{ grain_label }} value for {{ metric.label }} of {{ low_value|format_with_unit(metric.unit) "
        "}} is now the {%"
    ...
    StoryType.TOP_4_SEGMENTS: {
        "title": "Strongest segments ({{ dimension_label }})",
    ...
    StoryType.BOTTOM_4_SEGMENTS: {
        "title": "Weakest segments ({{ dimension_label }})",
    ...
    StoryType.SEGMENT_COMPARISONS: {
        "title": "{{ segment_a }} vs. {{ segment_b }} ({{ dimension_label }})",
    ...
    StoryType.NEW_STRONGEST_SEGMENT: {
        ...
        "}}% to an average {{ metric.label }} value of {{ previous_value|format_with_unit(metric.unit) }}.",
    },
    StoryType.NEW_WEAKEST_SEGMENT: {
        "title": "{{ segment_name }} is now the worst-performing segment ({{ dimension_label }})",
        "detail": "{{ segment_name }} is now the weakest performing segment with an average {{ metric.label }} value "
        "of {{ current_value|format_with_unit(metric.unit) }} -- {{ diff_from_avg_percent|format_percent }}% lower "
        "than the"
        "average across all segments of {{ avg_value|format_with_unit(metric.unit) }}. Meanwhile, the previously "
        "weakest"
        "segment {{ previous_segment }} is {{ trend_direction }} {{ change_percent|format_percent }}% to an "
        "average {{ metric.label }} value of {{ previous_value|format_with_unit(metric.unit) }}.",
    },
    StoryType.NEW_LARGEST_SEGMENT: {
        "title": "{{ segment_name }} is now the most represented segment ({{ dimension_label }})",
    ...
    StoryType.NEW_SMALLEST_SEGMENT: {
        "title": "{{ segment_name }} is now the least represented segment ({{ dimension_label }})",
    ...
    StoryType.FORECASTED_ON_TRACK: {
        ...
        "detail": "{{ metric.label }} is forecasted to end the {{ period }} at {{ forecasted_value|format_with_unit("
        "metric.unit)"
        "}} and beat its target of {{ target_value|format_with_unit(metric.unit) }} by {{ gap_percent|format_percent "
        "}}%",
    },
    StoryType.FORECASTED_OFF_TRACK: {
        ...
        "detail": "{{ metric.label }} is forecasted to end the {{ period }} at {{ forecasted_value|format_with_unit("
        "metric.unit)"
        "}} and miss its target of {{ target_value|format_with_unit(metric.unit) }} by {{ gap_percent|format_percent "
        "}}%",
    },
    StoryType.PACING_ON_TRACK: {
        ...
        "to end this {{ period }} at {{ projected_value|format_with_unit(metric.unit) }}, beating the target of {{ "
        "target_value|format_with_unit(metric.unit) }} by {{ gap_percent|format_percent }}% if the current trajectory "
        "holds.",
    },
    StoryType.PACING_OFF_TRACK: {
        "title": "Pacing to miss end of {{ period }} target by {{ gap_percent|format_percent }}%",
        "detail": "{{ percent_elapsed|format_percent }}% through the {{ period }}, {{ metric.label }} is pacing "
        "to end this {{ period }} at {{ projected_value|format_with_unit(metric.unit) }}, missing the target of {{ "
        "target_value|format_with_unit(metric.unit) }} by {{ gap_percent|format_percent }}% if the current trajectory "
        "holds.",
    },
