...
def create_story_group_dag(group: str, meta: dict[str, Any]) -> None:
    ...
    def story_group_dag():
        ...
        @task(task_id="prepare_story_alerts_commands")
        def prepare_story_alerts_commands(
            _tenants: list[int], _metric_ids_map: dict[str, list[str]], _grains_map: dict[str, list[str]]
        ) -> list[str]:
            logger.info("Preparing story alert commands for all tenants")
            commands = []
