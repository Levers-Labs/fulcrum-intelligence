...
class S3Client:
    ...
    async def upload_to_cloud_storage(self, file_path: str, file_key: str):
        """
        Asynchronously uploads a file to an S3 bucket.

        :param file_path: The local path to the file to be uploaded.
        :param file_key: The destination key within the S3 bucket.
        """
        # Use the aioboto3 session to create an asynchronous S3 client
        async with self.session.client("s3") as s3_client:
            # Asynchronously upload the file to the specified File key
            await s3_client.upload_file(file_path, self.bucket, file_key)
            logger.info("File %s uploaded to s3://%s/%s", file_path, self.bucket, file_key)

    async def generate_presigned_url(self, file_key: str, expiration=3600) -> str:
        """
        Generate a presigned URL to share an S3 object.

        :param file_key: The S3 object key for which to generate the presigned URL.
        ...
        return s3_client.generate_presigned_url(
            "get_object", Params={"Bucket": self.bucket, "Key": file_key}, ExpiresIn=expiration
        )
