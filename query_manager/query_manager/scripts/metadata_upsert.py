...
session_manager = init_session_manager(settings, app_name="query_metadata_upsert")


async def upsert_data(dimensions_file_path: str, metrics_file_path: str, tenant_id: int) -> None:
    """
...
async def main(tenant_id: int) -> None:
    ...
    try:
        await upsert_data(str(dimensions_file_path), str(metrics_file_path), tenant_id)
    except Exception as e:
        logger.error(f"Error updating metric dimensions: {str(e)}")
        raise
    finally:
        # clear context
        reset_context()
