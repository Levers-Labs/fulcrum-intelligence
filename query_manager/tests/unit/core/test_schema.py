...
async def test_dimension_create(async_db_session):
    ...
    async for session in async_db_session:
        result = await DimensionCreate.create(session, dimension_data)
        ...
        db_dimension = await session.execute(select(Dimension).filter_by(dimension_id="test_dimension"))
...
@pytest.mark.asyncio
async def test_dimension_update(async_db_session):
    async for session in async_db_session:
        # Prepare test data
        dimension_data = {
        ...
        result = await DimensionUpdate.update(session, instance, update_data)
        ...
        # Verify the result in the database
        db_dimension = await session.execute(select(Dimension).filter_by(dimension_id="test_dimension_update"))
        db_dimension = db_dimension.scalar_one()
        assert db_dimension is not None
...
@pytest.mark.asyncio
async def test_metric_create(async_db_session):
    async for session in async_db_session:
        # Prepare test data
        metric_data = {
        ...
        # Verify the result in the database
        db_metric = await session.execute(select(Metric).filter_by(metric_id="test_metric"))
...
@pytest.mark.asyncio
async def test_metric_update(async_db_session):
    async for session in async_db_session:
        # Prepare test data
        metric_data = {
        ...
        # Call the update method
        result = await MetricUpdate.update(session, instance, update_data)
        ...
        # Verify the result in the database
        db_metric = await session.execute(select(Metric).filter_by(metric_id="test_metric_update"))
        db_metric = db_metric.scalar_one()
        assert db_metric is not None
