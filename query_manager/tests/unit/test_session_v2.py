"""Tests for Session v2 implementation in query_manager."""
...
from query_manager.db.config import get_async_session, get_batch_session
...
def test_db_stats_endpoint_includes_session_count():
    ...
    mock_manager = MagicMock()
    mock_manager.current_session_count = 8
    ...
    with patch("query_manager.db.config.get_session_manager", return_value=mock_manager), patch(
        "query_manager.core.dependencies.oauth2_auth"
    ) as mock_oauth:
        ...
        try:
            ...
            response = client.get("/query/v1/health/db-stats")
...
async def test_session_manager_initialization():
    ...
    with patch("query_manager.lifespan.build_engine_options") as mock_build_opts, patch(
        "query_manager.lifespan.AsyncSessionManager"
    ) as mock_manager_class, patch("query_manager.lifespan.get_settings") as mock_get_settings:
