...
from query_manager.db.config import get_async_session, get_session


@pytest.fixture
def mock_sync_session(mocker):
    return mocker.patch("query_manager.db.config._get_session", return_value=Session())


@pytest.fixture
def mock_async_session(mocker):
    return mocker.patch("query_manager.db.config._get_async_session", return_value=AsyncSession())
